/**
 * Phase 4b.2: Mass Fuzzing Test Runner
 *
 * Reads test vectors from JSON file generated by Lean and validates
 * against our C implementation.
 *
 * Usage: ./test_fuzz <vectors.json>
 */
...
// Partial round: AddRC (first only) -> S-box (first only) -> Internal MDS
...
static bool parse_hex(const char** pp, bn254_fe* out) {
    ...
    // Parse hex digits (big-endian input)
    while ((*p >= '0' && *p <= '9') || (*p >= 'a' && *p <= 'f') || (*p >= 'A' && *p <= 'F')) {
        ...
        // Shift all limbs left by 4 bits and add digit
...
static void print_hex(const bn254_fe* v) {
    ...
    bool started = false;
    for (int i = 3; i >= 0; i--) {
        if (started || v->limbs[i] != 0 || i == 0) {
            if (started) {
                printf("%016llx", (unsigned long long)v->limbs[i]);
            } else {
                printf("%llx", (unsigned long long)v->limbs[i]);
                started = true;
            }
        }
    }
...
int main(int argc, char** argv) {
    ...
    printf("=================================================================\n");
    printf("     PHASE 4b.2: Lean -> C Fuzzing Validation                    \n");
    printf("=================================================================\n\n");

    const char* p = strstr(json, "\"vectors\"");
    if (!p) {
        fprintf(stderr, "Error: No vectors array found\n");
        free(json);
        return 1;
    }
    ...
    while (*p) {
        ...
        // Convert input to Montgomery form and run permutation
        ...
        // Convert output from Montgomery form
        bn254_fe actual[3];
        for (int i = 0; i < 3; i++) {
        ...
        if (match) {
            ...
            if (total <= 5 || total % 25 == 0) {
                printf("Vector %3d: PASS\n", total);
            }
        } else {
            failed++;
            printf("\n==== MISMATCH at vector %d ====\n", total);
            printf("Input:\n");
            for (int i = 0; i < 3; i++) {
    ...
    if (failed > 0) {
        ...
        printf("\n");
        printf("ALL TESTS PASSED - Lean spec matches C implementation\n");
        printf("=================================================================\n");
        return 0;
    }
}
