/**
 * Poseidon2 Test Vector Validation
 * Phase 4a: Validate against HorizenLabs reference
 *
 * Test Vector (from poseidon2-rs):
 *   Input:  [0, 1, 2]
...
/* ============================================================================
 * Constants from Lean-generated poseidon_constants.h
 * Note: RC values extracted here to avoid header conflicts
 * ============================================================================ */

// External reference to round constants (defined in poseidon_constants_data.c)
// For now, inline the first few and last few rounds for testing
static const uint64_t RC_3[64][3][4] = {
    ...
    // Rounds 4-59: partial (56 rounds) - using constants from generated file
...
// Debug: print state
static void debug_print_state(const char *label, poseidon_state_3 *state, const field_params *p) {
#ifdef DEBUG
    bn254_fe tmp;
    printf("%s:\n", label);
    for (int i = 0; i < 3; i++) {
        bn254_from_mont(&tmp, &state->elem[i], p);
        printf("  [%d]: ", i);
        bn254_print("", &tmp);
    }
#else
    (void)label; (void)state; (void)p;
#endif
}

// Add round constants (RC values from HorizenLabs are in STANDARD form, need Montgomery conversion)
static void add_round_constants(poseidon_state_3 *state, int round, const field_params *p) {
    for (int i = 0; i < 3; i++) {
        bn254_fe rc;
        bn254_from_limbs(&rc, RC_3[round][i]);
        // BUG FIX: RC values are in standard form, must convert to Montgomery form
        // State is in Montgomery form, so RC must also be in Montgomery form for correct addition
        bn254_to_mont(&rc, &rc, p);
        bn254_add(&state->elem[i], &state->elem[i], &rc, p);
    }
}

// External MDS for full rounds: circ(2,1,1) = [[2,1,1], [1,2,1], [1,1,2]]
// x'[i] = x[i] + sum
static void mds_external(poseidon_state_3 *state, const field_params *p) {
    // Compute sum = x[0] + x[1] + x[2]
    bn254_fe sum;
    bn254_add(&sum, &state->elem[0], &state->elem[1], p);
    bn254_add(&sum, &sum, &state->elem[2], p);

    // x'[i] = x[i] + sum (for all i)
...
// Internal MDS for partial rounds: [[2,1,1], [1,2,1], [1,1,3]]
// x'[0] = x[0] + sum, x'[1] = x[1] + sum, x'[2] = 2*x[2] + sum
static void mds_internal(poseidon_state_3 *state, const field_params *p) {
    ...
    // x'[2] = 2*x[2] + sum
    bn254_fe tmp2;
    bn254_add(&tmp2, &state->elem[2], &state->elem[2], p);  // 2*x[2]
...
// Full round: AddRC -> S-box (all) -> External MDS
static void full_round(poseidon_state_3 *state, int round, const field_params *p) {
#ifdef DEBUG
    if (round == 0) {
        printf("  Round 0 detailed trace:\n");
        debug_print_state("    Before AddRC", state, p);
    }
#endif
    add_round_constants(state, round, p);
#ifdef DEBUG
    if (round == 0) {
        debug_print_state("    After AddRC", state, p);
    }
#endif
    bn254_sbox5_full_round(state, p);
#ifdef DEBUG
    if (round == 0) {
        debug_print_state("    After S-box", state, p);
    }
#endif
    mds_external(state, p);
#ifdef DEBUG
    if (round == 0) {
        debug_print_state("    After MDS", state, p);
    }
#endif
}

// Partial round: AddRC (first element only) -> S-box (first only) -> Internal MDS
static void partial_round(poseidon_state_3 *state, int round, const field_params *p) {
    // In partial rounds, only add round constant to the FIRST element
    bn254_fe rc;
...
static void poseidon2_permutation(poseidon_state_3 *state, const field_params *p) {
    ...
    debug_print_state("Before initial MDS", state, p);

    // INITIAL external linear layer (before any rounds)
    mds_external(state, p);

    debug_print_state("After initial MDS", state, p);

    // First RF/2 = 4 full rounds
    for (int i = 0; i < 4; i++) {
        full_round(state, round++, p);
    }
#ifdef DEBUG
    debug_print_state("After 4 full rounds", state, p);
#endif

    // RP = 56 partial rounds
    for (int i = 0; i < 56; i++) {
        partial_round(state, round++, p);
#ifdef DEBUG
        if (i == 0 || i == 10 || i == 20 || i == 30 || i == 40 || i == 50 || i == 55) {
            printf("After partial round %d (round %d):\n", i, round-1);
            debug_print_state("", state, p);
        }
#endif
    }
...
#ifdef DEBUG
    debug_print_state("Before last 4 full rounds (after all partial)", state, p);
#endif
    for (int i = 0; i < 4; i++) {
        full_round(state, round++, p);
#ifdef DEBUG
        printf("After final full round %d (round %d):\n", i, round-1);
        debug_print_state("", state, p);
#endif
    }
}
...
int main(void) {
    printf("╔════════════════════════════════════════════════════════════╗\n");
    printf("║     PHASE 4a: Test Vector Validation                       ║\n");
    printf("╚════════════════════════════════════════════════════════════╝\n\n");

    field_params p;
    bn254_init_params(&p);

    // Initialize state with [0, 1, 2]
    poseidon_state_3 state;
    bn254_from_u64(&state.elem[0], 0, &p);
    bn254_from_u64(&state.elem[1], 1, &p);
    bn254_from_u64(&state.elem[2], 2, &p);

    printf("Input state:\n");
    bn254_fe tmp;
    bn254_from_mont(&tmp, &state.elem[0], &p);
    bn254_print("  [0]", &tmp);
    bn254_from_mont(&tmp, &state.elem[1], &p);
    bn254_print("  [1]", &tmp);
    bn254_from_mont(&tmp, &state.elem[2], &p);
    bn254_print("  [2]", &tmp);
    printf("\n");

    // Run permutation
    printf("Running Poseidon2 permutation (64 rounds)...\n\n");
    poseidon2_permutation(&state, &p);

    // Convert output from Montgomery form and print
    bn254_fe output[3];
    for (int i = 0; i < 3; i++) {
        bn254_from_mont(&output[i], &state.elem[i], &p);
    }

    printf("Output state:\n");
    bn254_print("  [0]", &output[0]);
    bn254_print("  [1]", &output[1]);
    bn254_print("  [2]", &output[2]);
    printf("\n");

    // Expected output (from HorizenLabs) - in standard form
    // perm[0] = 0x0bb61d24daca55eebcb1929a82650f328134334da98ea4f847f760054f4a3033
    bn254_fe expected[3];
    uint64_t exp0[4] = {0x47f760054f4a3033ULL, 0x8134334da98ea4f8ULL,
                        0xbcb1929a82650f32ULL, 0x0bb61d24daca55eeULL};
    uint64_t exp1[4] = {0x92defe7ff8d03570ULL, 0x77a15d3f74ca6549ULL,
                        0xcbcc80214f26a302ULL, 0x303b6f7c86d043bfULL};
    uint64_t exp2[4] = {0x86296242cf766ec8ULL, 0xe660b145994427ccULL,
                        0xf8617361c3ba7c52ULL, 0x1ed25194542b12eeULL};
    bn254_from_limbs(&expected[0], exp0);
    bn254_from_limbs(&expected[1], exp1);
    bn254_from_limbs(&expected[2], exp2);

    printf("Expected output:\n");
    bn254_print("  [0]", &expected[0]);
    bn254_print("  [1]", &expected[1]);
    bn254_print("  [2]", &expected[2]);
    printf("\n");

    // Validate
    bool pass = true;
    for (int i = 0; i < 3; i++) {
        if (!bn254_eq(&output[i], &expected[i])) {
    ...
    printf("════════════════════════════════════════════════════════════\n");
    if (pass) {
        printf("✓ TEST PASSED: Output matches HorizenLabs reference\n");
    } else {
        printf("✗ TEST FAILED: Output does not match expected values\n");
        printf("\nNote: If this fails, check:\n");
        printf("  1. Round constant ordering (row-major vs column-major)\n");
        printf("  2. MDS matrix layout\n");
        printf("  3. Montgomery conversion\n");
    }
    printf("════════════════════════════════════════════════════════════\n");
