...
/-! ## Phase 3: Poseidon2 Kernel Expansions -/

/-- Expand MDS matrix multiplication (symbolic - generates loop in CodeGen).
    For expansion purposes, we use identity as placeholder.
    Actual code is generated by Poseidon.CodeGen.genMDSApply -/
def expandMDSApply (_mdsName : String) (size : Nat) : ExpandedKernel :=
  -- MDS is a dense t×t matrix multiplication
  -- In scalar expansion, we'd need t² multiplications + t² additions
  -- But we keep it symbolic here; CodeGen handles the actual expansion
  let inputs := List.range size |>.map ScalarVar.input
  let outputs := List.range size |>.map ScalarVar.output
  -- Placeholder: identity (actual code generated by CodeGen)
  let body := List.range size |>.map fun i =>
    { target := .output i, value := ScalarExpr.x i }
  { inputVars := inputs, outputVars := outputs, body := body }

/-- Expand MDS internal (Poseidon2 optimization).
    M_I has structure: diag + rank-1 update
    y[i] = diag[i] * x[i] + sum(x[j]) -/
def expandMDSInternal (size : Nat) : ExpandedKernel :=
  let inputs := List.range size |>.map ScalarVar.input
  let outputs := List.range size |>.map ScalarVar.output
  -- Compute sum of all inputs first, then apply diagonal
  -- t0 = x0 + x1 + ... + x_{t-1}
  -- y_i = diag_i * x_i + t0  (simplified: we use 1 as diag for now)
  let body :=
    -- First, compute sum (using temp vars)
    let sumSteps := if size > 0 then
      [{ target := .temp 0, value := ScalarExpr.x 0 }] ++
      (List.range (size - 1) |>.map fun i =>
        { target := .temp 0, value := .add (.t 0) (ScalarExpr.x (i + 1)) })
    else []
    -- Then, output = x_i + sum (simplified: diag=1)
    let outputSteps := List.range size |>.map fun i =>
      { target := .output i, value := .add (ScalarExpr.x i) (.t 0) }
    sumSteps ++ outputSteps
  { inputVars := inputs, outputVars := outputs, body := body }
...
def expandAddRoundConst (round : Nat) (size : Nat) : ExpandedKernel :=
  ...
  -- Actual constant lookup is done by CodeGen
  let body := List.range size |>.map fun i =>
