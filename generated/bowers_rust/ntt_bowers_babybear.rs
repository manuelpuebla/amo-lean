...
//!   rustc -O ntt_bowers_babybear.rs -o ntt_bench && ./ntt_bench
//!
...
fn main() {
    let p: u64 = 2013265921;
    let sizes: &[(usize, usize)] = &[
        (1 << 14, 200), (1 << 16, 50), (1 << 18, 20), (1 << 20, 10), (1 << 22, 3),
    ];
    println!("AMO-Lean Bowers NTT — BabyBear (p = 2^31 - 2^27 + 1)");
    println!("Reduction: Solinas fold (e-graph selected, verified in Lean 4)");
    println!("Build: rustc -O ntt_bowers_babybear.rs -o ntt_bench && ./ntt_bench");
    println!();
    println!("{:>8} {:>12} {:>12} {:>10}", "N", "Time (us)", "Melem/s", "Iters");
    println!("{:>8} {:>12} {:>12} {:>10}", "--------", "----------", "---------", "-----");
    for &(n, iters) in sizes {
        let twiddles = compute_bowers_twiddles(n, 31);
