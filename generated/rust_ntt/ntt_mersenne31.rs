...
fn butterfly(a: &mut u32, b: &mut u32, w: u32) {
    ...
    *b = solinas_fold((2147483647 as u64).wrapping_add(orig_a as u64).wrapping_sub(wb as u64));
}

fn ntt_mersenne31(data: &mut [u32], twiddles: &[u32]) {
    let n = data.len();
    for stage in 0..20 {
        let half = 1 << (19 - stage);
        let mut group = 0;
