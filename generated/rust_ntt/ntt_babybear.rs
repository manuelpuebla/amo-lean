...
fn solinas_fold(x: u64) -> u32 {
    (((x >> 31) as u64).wrapping_mul(134217727 as u64))
...
fn butterfly(a: &mut u32, b: &mut u32, w: u32) {
    ...
    *b = solinas_fold((2013265921 as u64).wrapping_add(orig_a as u64).wrapping_sub(wb as u64));
}

fn ntt_babybear(data: &mut [u32], twiddles: &[u32]) {
    let n = data.len();
    for stage in 0..20 {
        let half = 1 << (19 - stage);
        let mut group = 0;
